...
public class FollowListPage
  extends FollowerListFakeActivity
  implements View.OnClickListener, AdapterView.OnItemClickListener
{
  ...
  public void onCreate()
  {
    ...
    int j = R.getStringRes(getContext(), "multi_share");
    if (j > 0) {
      this.llTitle.getTvTitle().setText(j);
    }
    this.llTitle.getBtnRight().setVisibility(0);
    int k = R.getStringRes(getContext(), "finish");
    if (k > 0) {
    ...
    int m = R.getBitmapRes(getContext(), "title_shadow");
    if (m > 0) {
